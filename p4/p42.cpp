    ...
    void insert(string value, int lineNum) {
        ...
        for (int i = 0; i<int(table.size()); i++) {
            if (value == table[i].value) {
                table[i].lineNums.push_back(lineNum);
                repeat = true;
            }
        }
        ...
        while(!(table[index].value.empty())) { //checks if the initial index is empty
                ...
                collisions++;
                cout<<index<<": "<<value<<": "<<collisions<<endl;
            }
...
int main(int argc, char* argv[]) {
   ...
   while (getline(inputFile, line)) {
    ...
    inputFile.close();
   cout<<"The number of words found in the file was "<<numWords+1<<endl;
   cout<<"The number of unique words found in the file was "<<hash.getUniques()-1<<endl;
